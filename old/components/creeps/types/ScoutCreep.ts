            ...
            if(station) {
                if (creep.room.name === station) {
                    ...
                    if(!creep.memory.saidHello) {
                        let hasAllySpawn = creep.room.find(FIND_HOSTILE_SPAWNS, {filter: (c: Spawn) => _.indexOf(Config.FRIENDS, c.owner.username) >= 0}).length > 0;
                        creep.say(hasAllySpawn ? "Doing ok?" : "Hello!");
